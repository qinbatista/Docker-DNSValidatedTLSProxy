# DNS-validated TLS proxy for `la.qyp.life`

This is a separate Docker Compose project for the existing `video-url-download` service. It builds Caddy with the Route 53 DNS module, gets its certificate through ACME DNS-01, and proxies to `video-url-download:8787` over that service's existing Docker network.
...
- The Route 53 principal in `.env` needs permission to create and remove the temporary `_acme-challenge.la.qyp.life` TXT record. Do not put those credentials in GitHub or in this repository.
...
```text
cp .env.example .env
# Edit .env with the real ACME email and the restricted Route 53 credentials.
docker compose pull
docker compose up -d
docker compose ps
```
