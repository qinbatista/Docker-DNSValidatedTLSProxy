...
The Pi's `/opt/qyp-life-tls-proxy` directory was based on commit `b694522` with uncommitted Caddy and Compose changes; it is not byte-identical to this repository's main branch. For an exact 2026-09-24 source recovery, use the private `qinbatista/Docker-DNSValidatedTLSProxy-PiRecovery` snapshot (`c8f626b`) and the runtime `.env` from `mypi-docker-core-20260924T0426Z.tar.gz`. Treat a fresh `latest` image pull as a new binary until its behavior is checked.
